...
export default class ProductRepositoryDatabase implements ProductRepository {
	...
	async getProducts(): Promise<Product[]> {
		const productsData = await this.connection.query("select * from cccat10.product where width > 0 and height > 0 and length > 0 and weight > 0 order by id_product asc", []);
		const products = [];
