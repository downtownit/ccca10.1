    ...
    const checkoutGateway: CheckoutGateway = {
		async getProducts (): Promise<Product[]> {
			return [
				...
				new Product(3, "C", 30),
                new Product(5, "A", 1000)
			];
		},
		async checkout (order: Order): Promise<any> {
			return { total: 6090, freight: 280 }
		}
	}
    const wrapper = mount(CheckoutViewVue, { global: { provide: { checkoutGateway }} });

    expect(wrapper.get(".title-name").text()).toBe("Checkout in processing!");
    await sleep (200);
